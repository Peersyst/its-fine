...
function useContextMap(ignoredContexts = []) {
  const fiber = useFiber();
  const [contextMap] = React__namespace.useState(() => /* @__PURE__ */ new Map());
  const ignoredContextsSet = React__namespace.useMemo(() => new Set(ignoredContexts), [ignoredContexts]);
  contextMap.clear();
  let node = fiber;
  while (node) {
    if (node.type && typeof node.type === "object") {
      ...
      if (context && context !== FiberContext && (!context.displayName || !ignoredContextsSet.has(context.displayName)) && !contextMap.has(context)) {
...
function useContextBridge(ignoredContexts = []) {
